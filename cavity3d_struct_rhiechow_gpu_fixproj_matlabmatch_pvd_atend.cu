...
struct Params
{
  ...
  int write_every = 0;
  int write_initial = 0;
  std::string output_prefix = "cavity3d_struct_rhiechow_gpu";
  int vel_maxit = 200;
...
static void ParseArgs(int argc, char **argv, Params &par)
{
  for (int a = 1; a < argc; ++a) {
    ...
    else if (!std::strcmp(argv[a], "-write-every") && a + 1 < argc) par.write_every = std::atoi(argv[++a]);
    else if (!std::strcmp(argv[a], "-write-initial") && a + 1 < argc) par.write_initial = std::atoi(argv[++a]);
    else if (!std::strcmp(argv[a], "-output-prefix") && a + 1 < argc) par.output_prefix = argv[++a];
...
__global__ static void BuildPressureRhsKernel(
    int N, int ref_idx, const double *divU, double *rhs)
{
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
static std::string snapshot_vtk_name(const std::string &prefix, int step)
{
  std::ostringstream oss;
  oss << prefix << "_step" << std::setw(7) << std::setfill('0') << step << ".vtk";
  return oss.str();
}
...
static void write_pvd_collection(
    const std::string &pvd_file,
    const std::vector<int> &steps,
    const std::vector<double> &times,
    const std::vector<std::string> &files)
{
  std::ofstream out(pvd_file.c_str());
  out << "<?xml version=\"1.0\"?>\n";
  out << "<VTKFile type=\"Collection\" version=\"0.1\" byte_order=\"LittleEndian\">\n";
  out << "  <Collection>\n";
  for (std::size_t i = 0; i < files.size(); ++i) {
    const std::string relfile = path_basename(files[i]);
    out << "    <DataSet timestep=\"" << std::setprecision(16) << times[i]
        << "\" group=\"\" part=\"0\" file=\"" << relfile << "\"/>\n";
  }
  out << "  </Collection>\n";
  out << "</VTKFile>\n";
}
...
int main(int argc, char **argv)
{
  ...
  if (rank == 0) {
    ...
    std::printf("MATLAB-consistent IPCS/Rhie-Chow collocated solve using aP_inv.\n");
    ...
    std::printf("write_every   : %d\n", par.write_every);
    if (par.write_every > 0) std::printf("output_prefix : %s\n", par.output_prefix.c_str());
    std::printf("=====================================================================\n");
  }
  ...
  HYPRE_CALL(HYPRE_StructPFMGSetNumPreRelax(p_prec, 1));
  HYPRE_CALL(HYPRE_StructPFMGSetNumPostRelax(p_prec, 1));
  ...
  auto write_snapshot = [&](int step, double time_value) {
    CUDA_CALL(cudaDeviceSynchronize());
    if (rank == 0) {
      std::string vtk_name = snapshot_vtk_name(par.output_prefix, step);
      write_vtk_cell_centered(vtk_name, nx, ny, nz, dx, dy, dz, u, v, w, p);
      snapshot_steps.push_back(step);
      snapshot_times.push_back(time_value);
      snapshot_files.push_back(vtk_name);
      std::printf("Wrote snapshot: %s (t=%.6e)\n", vtk_name.c_str(), time_value);
    }
  };

  if (par.write_every > 0 && par.write_initial) {
    write_snapshot(0, 0.0);
  }
  ...
  for (int step = 1; step <= par.nsteps; ++step) {
    ...
    if (par.write_every > 0 && (step % par.write_every == 0 || step == par.nsteps)) {
      const double time_value = step * par.dt;
      write_snapshot(step, time_value);
    }
  }
  ...
  if (rank == 0 && !snapshot_files.empty()) {
    const std::string pvd_name = par.output_prefix + ".pvd";
    write_pvd_collection(pvd_name, snapshot_steps, snapshot_times, snapshot_files);
    std::printf("Wrote PVD file: %s\n", pvd_name.c_str());
  }
